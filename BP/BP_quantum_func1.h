...
bool  quan_decode(GF2mat &H, GF2mat &H2,const nodes checks[],const nodes errors[],const vec &pv,const vec&pv_dec,double pavg,double range,double& num_iter, int lmax,int& wt,int &max_fail, int &syn_fail, int debug, vec &LR,int rankH,int &OSD_sec,double alpha=1,double lambda=1);
